...
class StreamSegment:
    def __init__(self, seg_id, len_km=0, area_sqkm=0, tot_area_sqkm=0, proc_unit=''):
        ...
        self.all_parents = {}
        self.proc_unit = str(proc_unit)
        
    def update_data(self, len_km=0, area_sqkm=0, tot_area_sqkm=0, proc_unit=''):
        self.len_km = float(len_km)
        self.area_sqkm = float(area_sqkm)
        self.tot_area_sqkm = float(tot_area_sqkm)
        self.proc_unit = str(proc_unit)
...
def upstream_setup(df_infile):
    ...
    for row in df.itertuples():
        ...
        if not seg_id in segments:
            ...
            new_seg = StreamSegment(seg_id, len_km, area_sqkm, tot_area_sqkm, proc_unit)
            ...
            seg.update_data(len_km, area_sqkm, tot_area_sqkm, proc_unit)
...
def upstream_build_network(traverse_queue):
#def upstream_build_network(traverse_queue,hdf_name):
    ...
    while traverse_queue_indx < len(traverse_queue):
        ...
        hdf_name = 'data/nhdplusv2_' + str(seg.proc_unit) + '_network.h5'
        f = h5py.File(hdf_name, 'a')
